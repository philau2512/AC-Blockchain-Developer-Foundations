...
contract NFTSaleWhitelistV2 is ERC721, Ownable {
    ...
    // Thay thế mapping whitelist bằng merkleRoot
    ...
    function mint(
        uint256 amount,
        bytes32[] calldata merkleProof
    ) external payable onlyWhitelisted(merkleProof) {
        require(amount > 0, "Amount must be greater than 0");
        require(
            mintedPerWallet[msg.sender] + amount <= maxPerWallet,
            "Exceeds the maximum number of tokens per wallet"
        );
